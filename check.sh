...
signature=$(grep -cP '^PASS_MAX_DAYS\s+365$' /etc/login.defs)
if [ $signature -eq 0 ];
then
  status="\e[91m[ BAD ]"
  #exit
...
signature=$(grep -cP '^PASS_MIN_DAYS\s+360$' /etc/login.defs)
if [ $signature -eq 0 ];
then
  status="\e[91m[ BAD ]"
  #exit
...
signature=$(grep -cP '^PASS_WARN_AGE\s+350$' /etc/login.defs)
if [ $signature -eq 0 ];
then
  status="\e[91m[ BAD ]"
  #exit
...
signature=$(grep -cP '.*minlen=12.*' /etc/pam.d/common-password)
if [ $signature -eq 0 ];
then
  status="\e[91m[ BAD ]"
  #exit
...
signature=$(grep -cP '.*minclass=3.*' /etc/pam.d/common-password)
if [ $signature -eq 0 ];
then
  status="\e[91m[ BAD ]"
  #exit
...
signature=$(grep -cP '.*dcredit=1 lcredit=1 ocredit=1 ucredit=1.*' /etc/pam.d/common-password)
if [ $signature -eq 0 ];
then
  status="\e[91m[ BAD ]"
  #exit
...
signature=$(grep -cP '.*maxrepeat=3.*' /etc/pam.d/common-password)
if [ $signature -eq 0 ];
then
  status="\e[91m[ BAD ]"
  #exit
